// -----------------------------------------------------------------------
// <copyright file="CreateReportSchedule.cs" company="">
...
namespace ssrsmsbuildtasks.RS2008R2
{
    ...
    public class CreateReportSchedule : Task
    {
        ...
        {
            vaildScheduleRecurrence = new List<string>() { "Min", "Daily", "Weekly", "Monthly", "MonthlyDOW" };
            vaildDaysOfWeek = new List<string>() { "Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun" };
            vaildMonthsOfYear = new List<string>(){"Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"};
        }
        ...
        public override bool Execute()
        {
            ...
            try
            {
                for (int index = 0; index < this.ReportSchedules.Length; index++)
                {
                    ...
                    if(!DateTime.TryParse(this.ReportSchedules[index].GetMetadata("StartDateTime"), out reportScheduleDateTime))
                    {
                        throw new FormatException(string.Format("Report Schedule {0} StartDateTime is not vaild ",this.ReportSchedules[index].ItemSpec));
                    }
                    ...
                    if (!string.IsNullOrEmpty(this.ReportSchedules[index].GetMetadata("EndDateTime")))
                    {
                                ...
                                this.ReportSchedules[index].GetMetadata("EndDateTime"), out reportScheduleDateTime))
                        {
                            throw new FormatException(
                                string.Format(
                                    "Report Schedule {0} EndDateTime is not vaild ", this.ReportSchedules[index].ItemSpec));
                        }
                        reportSchedules[index].EndDateTime = reportScheduleDateTime;
                    }
                    reportSchedules[index].ScheduleRecurrence = (ScheduleRecurrenceEnum) Enum.Parse( typeof(ScheduleRecurrenceEnum), this.ReportSchedules[index].GetMetadata("ScheduleRecurrence"), true);

                    switch (reportSchedules[index].ScheduleRecurrence)
                    {
                            case ScheduleRecurrenceEnum.Min:
                            case ScheduleRecurrenceEnum.Daily:
                            reportSchedules[index].Interval =
                                this.ReportSchedules[index].GetMetadata("Interval");
                            break;
                            case ScheduleRecurrenceEnum.Weekly: 
                                GetDaysOfWeek(reportSchedules[index].Days, this.ReportSchedules[index].GetMetadata("DaysOfWeek"));
                            ...
                            case ScheduleRecurrenceEnum.Monthly:
                                GetMonthsOfYear(reportSchedules[index].Months, this.ReportSchedules[index].GetMetadata("MonthsOfYear"));
                                if(!string.IsNullOrEmpty(this.ReportSchedules[index].GetMetadata("Interval")))
                                {
                                    reportSchedules[index].Interval = this.ReportSchedules[index].GetMetadata("Interval");
                                }
        ...
        /// <param name="monthsOfYearString">The months of year string.</param>
        private void GetMonthsOfYear(ReportScheduleMonths months, string monthsOfYearString)
        {
            foreach (string monthOfYear in monthsOfYearString.Split(';'))
            {
                switch (monthOfYear)
                {
                    case "Jan": months.Jan = true;
                        break;
                    case "Feb": months.Feb = true;
                        break;
                    case "Mar": months.Mar = true;
                        break;
                    case "Apr": months.Apr = true;
                        break;
                    case "May": months.May = true;
                        break;
                    case "Jun": months.Jun = true;
                        break;
                    case "Jul": months.Jul = true;
                        break;
                    case "Aug": months.Aug = true;
                        break;
                    case "Sep": months.Sep = true;
                        break;
                    case "Oct": months.Oct = true;
                        break;
                    case "Nov": months.Nov = true;
                        break;
                    case "Dec": months.Dec = true;
                        break;
                }
            }
        }
        ...
        /// </summary>
        /// <param name="days">The of week string.</param>
        /// <param name="daysOfWeekString">The days of week string.</param>
        ...
        /// <param name="invaildDataourceMessage">The invaild dataource message.</param>
        /// <returns>
        ...
        private bool isVaildReportSchedule(ITaskItem reportSchedule, out string invaildDataourceMessage)
        {
            ...
            else
            {
                ...
                if (!vaildScheduleRecurrence.Any(e => e.Equals(reportSchedule.GetMetadata("ScheduleRecurrence"))))
                {
                    string vaildScheduleRecurrenceToString = vaildScheduleRecurrence.Aggregate((workingString, scheduleRecurrence) => string.Format("{0},{1}", workingString, scheduleRecurrence));
                    isVaild = false;
                    invalidReportScheduleMessageStringBuilder.AppendLine(
                        string.Format("{0}:ScheduleRecurrence is not vaild (vaild values are: {1} )", reportSchedule.ItemSpec, vaildScheduleRecurrenceToString));
                }
                ...
                switch (reportSchedule.GetMetadata("ScheduleRecurrence"))
                {
                    ...
                    case "Daily":
                        ...
                        break;
                    case "Weekly":
                         isVaild = isVaild && this.hasVaildDayOfWeek(reportSchedule, invalidReportScheduleMessageStringBuilder, "Weekly");
                        break;
                    case "Monthly": 
                         isVaild = isVaild && this.hasInterval(reportSchedule, invalidReportScheduleMessageStringBuilder);
                         isVaild = isVaild && this.hasVaildMonths(reportSchedule, invalidReportScheduleMessageStringBuilder,"Monthly");
                        break;
                    case "MonthlyDOW":
                        isVaild = isVaild && this.hasVaildDayOfWeek(reportSchedule, invalidReportScheduleMessageStringBuilder, "MonthlyDOW");
                        isVaild = isVaild && this.hasVaildMonths(reportSchedule, invalidReportScheduleMessageStringBuilder, "MonthlyDOW");
                        break;
                }
        ...
        /// </summary>
        /// <param name="reportSchedule">The report schedule.</param>
        /// <param name="invalidReportScheduleMessageStringBuilder">The invalid report schedule message string builder.</param>
        /// <param name="recurrenceName">Name of the recurrence.</param>
        /// <returns>
        /// 	<c>true</c> if [has vaild months] [the specified report schedule]; otherwise, <c>false</c>.
        /// </returns>
        private bool hasVaildMonths(ITaskItem reportSchedule, StringBuilder invalidReportScheduleMessageStringBuilder, string recurrenceName)
        {
            bool isVaild = true;
            if (string.IsNullOrEmpty(reportSchedule.GetMetadata("MonthsOfYear")))
            {
                isVaild = false;
                invalidReportScheduleMessageStringBuilder.AppendLine(
                    string.Format("{0}:Missing MonthsOfYear Metadata for {1} Recurrence", reportSchedule.ItemSpec, recurrenceName));
            }
            else
            {
                if (!reportSchedule.GetMetadata("MonthsOfYear").Split(new char[] { ';' }).All(monthOfYear => this.vaildMonthsOfYear.Any(e => e.Equals(monthOfYear))))
                {
        ...
        /// </returns>
        private bool hasVaildDayOfWeek(ITaskItem reportSchedule, StringBuilder invalidReportScheduleMessageStringBuilder, string recurrenceName)
        {
            bool isVaild = true;
            if (string.IsNullOrEmpty(reportSchedule.GetMetadata("DaysOfWeek")))
            {
                isVaild = false;
                invalidReportScheduleMessageStringBuilder.AppendLine(
                    string.Format("{0}:Missing DaysOfWeek Metadata for {1} Recurrence", reportSchedule.ItemSpec, recurrenceName));
            }
            else
            {
                if (
                    !reportSchedule.GetMetadata("DaysOfWeek").Split(new char[] { ';' }).All(
